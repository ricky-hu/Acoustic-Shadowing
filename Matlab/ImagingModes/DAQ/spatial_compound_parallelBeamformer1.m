% function out = spatial_compound_parallelBeamformer(in, maxAprSz, Fs,angle)
Fs = 40e6;
angle= 2;
...
%             delays = ( (Z*cos(alpha)+X*sin(alpha)) + sqrt( Z^2 + (X-X1).^2 ) ) / c * Fs; 
            delays = ( (Z*cos(alpha)+X*sin(alpha)) ) / c * Fs+1; 
            delays = round(delays); % no interpolation used in this version
            chnumOfLines = zeros(numAngles,128);  % will be filled with proper signal from each channel
