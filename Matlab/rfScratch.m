...
% looking at characteristic window at row 400-420
patchX = 400;
patchY = 156;
% size of patch in the scanline direction, which is 3x the wavelength
patchSizeY = 50;
...
window = absHil((patchX - floor(patchSizeY/2)) : (patchX + floor(patchSizeY/2)) , (patchY - floor(patchSizeX/2)):(patchY + floor(patchSizeX/2)));

figure()
w(1) = histogram(window, 'Normalization', 'probability', 'binWidth', 5);
