...
for i=1:cols
    ...
    fitVals = linspace(0,maxVal,maxVal/sizeWindow);
    theta = mle(reshape(window(:,:,i),[],1),'distribution','rayl');
    ralyWindow = (fitVals/(theta^2)).*exp(-(fitVals.^2) ./ (2*theta^2));
    hold on;
    plot(ralyWindow)
