...
%% Edit History:
%% Apr 9 2018   rhu     Changed to .png and changed output to have same name as original .b8 with frame number appended
%% Date: Feb 17, 2016
%% Read a .b8 file (from Ultrasonix machine) and write all the frames as png in the fullFileName\jpgs folder
...
%    enhanceImArr = zeros(header.h,header.w,header.nframes);
    ...
    if(header.filetype == 4) %postscan B .b8
        ...
        for frame_count = 1:header.nframes
            ...
            % Changed output file to have same name and stored in same
            % directory instead of subdirectory with Im1, Im2 etc.
            % also changing output to be written as .png
			% ImOutputFile = strcat(outputDir, '\', baseFileName_NoExtension, '\Im', int2str(frame_count), '.jpg');
            
            ImOutputFile = strcat(outputDir, '\', baseFileName_NoExtension, '_', int2str(frame_count), '.png');
