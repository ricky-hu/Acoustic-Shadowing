...
disp(['Texo acquired ' invoke(h, 'texoGetCollectedFrameCount') ' frames of RF data which are available in the cine']);

%% Reading the data into the Matlab environment amd displaying
data = invoke(h, 'texoGetCine');
data = reshape(data, [lineSize/2 128 invoke(h, 'texoGetCollectedFrameCount')]);
% defining a header
